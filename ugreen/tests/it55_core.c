...
EXPORT_SYMBOL(ec_send_command);
EXPORT_SYMBOL(ec_send_data);
EXPORT_SYMBOL(ec_receive_data);
EXPORT_SYMBOL(ec_read_memory);
EXPORT_SYMBOL(ec_write_memory);
/* Module initialization */
static int __init ec_core_init(void)
{
    pr_info("EC core driver initialized\n");
    return 0;
}

/* Module cleanup */
static void __exit ec_core_exit(void)
{
    pr_info("EC core driver exited\n");
}

module_init(ec_core_init);
module_exit(ec_core_exit);

MODULE_AUTHOR("drivers@ugreen");
MODULE_DESCRIPTION("it55 Embedded Controller Core Driver");
MODULE_LICENSE("GPL");